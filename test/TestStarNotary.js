...
it("can Create a Star", async () => {
  ...
  await instance.createStar("Awesome Star!", tokenId, "LLW");
  assert.equal(
...
it("lets user1 put up their star for sale", async () => {
  ...
  await instance.createStar("awesome star", starId, "LLW", { from: user1 });
...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenId = 71;
  let instance = await StarNotary.deployed();
  await instance.createStar("Limitless Star!", tokenId, "LWW");
  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(
    (await instance.tokenIdToStarInfo.call(tokenId)).name,
    "Limitless Star!"
  );
  assert.equal((await instance.tokenIdToStarInfo.call(tokenId)).symbol, "LWW");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  let firstTokenId = 8;
  await instance.createStar("Limitless First Star!", firstTokenId, "WLW", {
    from: accounts[1],
  });

  let secondTokenId = 122;
  await instance.createStar("Limitless Second Star!", secondTokenId, "WWW", {
    from: accounts[2],
  });
...
it("lets a user transfer a star", async () => {
  ...
  // 1. create a Star with different tokenId
  let tokenId = 908;
  await instance.createStar("Limitless testing Star!", tokenId, "PPW", {
    from: starOwner,
  });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  // 1. create a Star with different tokenId
  let tokenId = 17;
  await instance.createStar("Star!", tokenId, "LTE", {
    from: starOwner,
  });
